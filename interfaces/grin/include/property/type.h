...
bool grin_insert_vertex_type_to_list(GRIN_GRAPH, GRIN_VERTEX_TYPE_LIST,
                                     GRIN_VERTEX_TYPE);

size_t grin_get_vertex_type_list_size(GRIN_GRAPH, GRIN_VERTEX_TYPE_LIST);

GRIN_VERTEX_TYPE grin_get_vertex_type_from_list(GRIN_GRAPH,
                                                GRIN_VERTEX_TYPE_LIST, size_t);
#endif
...
bool grin_insert_edge_type_to_list(GRIN_GRAPH, GRIN_EDGE_TYPE_LIST,
                                   GRIN_EDGE_TYPE);

size_t grin_get_edge_type_list_size(GRIN_GRAPH, GRIN_EDGE_TYPE_LIST);

GRIN_EDGE_TYPE grin_get_edge_type_from_list(GRIN_GRAPH, GRIN_EDGE_TYPE_LIST,
                                            size_t);
#endif
 ...
 * many-to-many. Thus GRIN returns the pairs of vertex types related to an edge
 * type as a pair of vertex type lists of the same size, and the src/dst vertex
 * types are aligned with their positions in the lists.
 */
///@{
#if defined(GRIN_WITH_VERTEX_PROPERTY) && defined(GRIN_WITH_EDGE_PROPERTY)
/** @brief  the src vertex type list */
GRIN_VERTEX_TYPE_LIST grin_get_src_types_from_edge_type(GRIN_GRAPH,
                                                        GRIN_EDGE_TYPE);

/** @brief get the dst vertex type list */
GRIN_VERTEX_TYPE_LIST grin_get_dst_types_from_edge_type(GRIN_GRAPH,
                                                        GRIN_EDGE_TYPE);

/** @brief get the edge type list related to a given pair of vertex types */
GRIN_EDGE_TYPE_LIST grin_get_edge_types_from_vertex_type_pair(GRIN_GRAPH,
                                                              GRIN_VERTEX_TYPE,
                                                              GRIN_VERTEX_TYPE);
#endif
///@}
