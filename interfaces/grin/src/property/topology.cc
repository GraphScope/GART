...
GRIN_VERTEX grin_get_vertex_from_original_id_by_type(GRIN_GRAPH g, GRIN_VERTEX_TYPE vt, GRIN_VERTEX_ORIGINAL_ID oid) {
  auto _g = static_cast<GRIN_GRAPH_T*>(g);
  auto _oid = static_cast<GRIN_VERTEX_ORIGINAL_ID_T*>(oid);  // oid equals to gid in gart
  auto v = new GRIN_VERTEX_T();
  if (_g->Gid2Vertex(*_oid, *v)) {
    return v;
  }
...
GRIN_VERTEX_LIST grin_select_type_for_vertex_list(GRIN_GRAPH g, GRIN_VERTEX_TYPE vt, GRIN_VERTEX_LIST vl) {
...
GRIN_ADJACENT_LIST grin_select_edge_type_for_adjacent_list(GRIN_GRAPH g, GRIN_EDGE_TYPE et, GRIN_ADJACENT_LIST adj_list) {
