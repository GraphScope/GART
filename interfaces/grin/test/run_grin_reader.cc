...
int main(int argc, char** argv) {
  grape::InitMPIComm();
  {
    ...
    auto inner_vertex_list = grin_select_master_for_vertex_list(fragment, vertex_list);
    auto outer_vertex_list = grin_select_mirror_for_vertex_list(fragment, vertex_list);
    /*
    while (!grin_is_vertex_list_end(fragment, vertex_iter)) {
      auto v = grin_get_vertex_from_iter(fragment, vertex_iter);
      auto _v = static_cast<GRIN_VERTEX_T*>(v);
      //std::cout << "vertex label = " << _g->vertex_label(*_v) << " offset = " << _g->vertex_offset(*_v) 
      //          << " is inner vertex = " << _g->IsInnerVertex(*_v) << " fid = " << _g->fid()<< std::endl;
    ...
    auto inner_vertex_iter = grin_get_vertex_list_begin(fragment, inner_vertex_list);
    while (!grin_is_vertex_list_end(fragment, inner_vertex_iter)) {
      ...
      //          << " offset = " << _g->vertex_offset(*_v) << " fid = " << _g->fid()<< std::endl;
      
      auto vertex_type = grin_get_vertex_type(fragment, v);
      auto vertex_property_table = grin_get_vertex_property_table_by_type(fragment, vertex_type);
      auto vertex_property_list = grin_get_vertex_property_list_by_type(fragment, vertex_type);
      size_t v_prop_size = grin_get_vertex_property_list_size(fragment, vertex_property_list);
      for (auto idx = 1; idx < v_prop_size; ++idx) {
        auto vertex_property = grin_get_vertex_property_from_id(fragment, vertex_type, idx);
        auto v_data = grin_get_value_from_vertex_property_table(fragment,vertex_property_table, v, vertex_property);
        auto v_data_type = grin_get_vertex_property_data_type(fragment, vertex_property);
        if (v_data_type == GRIN_DATATYPE::Int32) {
          //std::cout << " vdata = " << *(static_cast<int32_t*>(const_cast<void*>(v_data))) << std::endl;
        } else if (v_data_type == GRIN_DATATYPE::Double) {
          //std::cout << " vdata = " << *(static_cast<double*>(const_cast<void*>(v_data))) << std::endl;
        } else {
      ...
      auto vertex_property_row = grin_get_row_from_vertex_property_table(fragment, vertex_property_table, v, vertex_property_list);
      for (auto idx = 1; idx < v_prop_size; idx++) {
        auto vertex_property = grin_get_vertex_property_from_id(fragment, vertex_type, idx);
        auto v_data_type = grin_get_vertex_property_data_type(fragment, vertex_property);
        auto v_data = grin_get_value_from_row(fragment, vertex_property_row, v_data_type, idx);
        if (v_data_type == GRIN_DATATYPE::Int32) {
          //std::cout << "row vdata = " << *(static_cast<int32_t*>(const_cast<void*>(v_data))) << std::endl;
        } else if (v_data_type == GRIN_DATATYPE::Double) {
          //std::cout << "row vdata = " << *(static_cast<double*>(const_cast<void*>(v_data))) << std::endl;
        } else {
          //std::cout << "unsupported data type ..." << std::endl;
        }
        grin_destroy_vertex_property(fragment, vertex_property);
    ...
    auto outer_vertex_iter = grin_get_vertex_list_begin(fragment, outer_vertex_list);
    ...
    auto inner_select_vertex_list = grin_select_type_for_vertex_list(fragment, vtype, inner_vertex_list);
    auto inner_select_vertex_iter = grin_get_vertex_list_begin(fragment, inner_select_vertex_list);
    while (!grin_is_vertex_list_end(fragment, inner_select_vertex_iter)) {
      auto v = grin_get_vertex_from_iter(fragment, inner_select_vertex_iter);
      auto _v = static_cast<GRIN_VERTEX_T*>(v);
    ...
    */
    ...
    inner_vertex_list = grin_select_master_for_vertex_list(fragment, vertex_list);
    auto inner_vertex_iter = grin_get_vertex_list_begin(fragment, inner_vertex_list);
    while (!grin_is_vertex_list_end(fragment, inner_vertex_iter)) {
      auto v = grin_get_vertex_from_iter(fragment, inner_vertex_iter);
      auto _v = static_cast<GRIN_VERTEX_T*>(v);
      ...
      while (!grin_is_adjacent_list_end(fragment, edge_iter)) {
        ...
        auto edge_property_list = grin_get_edge_property_list_by_type(fragment, edge_type);
        auto edge_prop_row = grin_get_edge_row(fragment, edge, edge_property_list);
        size_t edge_prop_size = grin_get_edge_property_list_size(fragment, edge_property_list);
        
        for (auto idx = 0; idx < edge_prop_size; idx++) {
          auto edge_property = grin_get_edge_property_from_id(fragment, edge_type, idx);
          auto edge_data_type = grin_get_edge_property_data_type(fragment, edge_property);
          auto edge_data = grin_get_value_from_row(fragment, edge_prop_row, edge_data_type, idx);
          if (edge_data_type == GRIN_DATATYPE::Int32) {
            std::cout << " edata = " << *(static_cast<int32_t*>(const_cast<void*>(edge_data))) << std::endl;
          } else if (edge_data_type == GRIN_DATATYPE::Double) {
            std::cout << " edata = " << *(static_cast<double*>(const_cast<void*>(edge_data))) << std::endl;
          } else {
            std::cout << "unsupported data type ..." << std::endl;
          }
        ...
        std::cout << "src vertex label = " << _g->vertex_label(*_v) << " offset = " << _g->vertex_offset(*_v) 
               << " dst vertex label = " << _g->vertex_label(*_nbr) << " offset = "  << _g->vertex_offset(*_nbr) 
               << " fid = " << _g->fid() << "edge prop size = " << edge_prop_size << std::endl;
    ...
    grin_destroy_vertex_list(fragment, vertex_list);
    grin_destroy_vertex_list(fragment, inner_vertex_list);
    std::cout << "vertex num = " << grin_get_vertex_num(fragment) << " fid = " << _g->fid() << std::endl;
    std::cout << "edge num = " << grin_get_edge_num(fragment) << " fid = " << _g->fid() << std::endl;
