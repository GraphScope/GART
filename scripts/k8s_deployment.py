...
if __name__ == "__main__":
    ...
    if False:
        launch_etcd_pod_deployment_cmd = (
            f"kubectl apply -f {gart_home}/k8s/etcd-deployment.yaml"
        )
        ...
        launch_etcd_pod_service_cmd = (
            f"kubectl apply -f {gart_home}/k8s/etcd-service.yaml"
        )
        process = subprocess.run(
            launch_etcd_pod_service_cmd,
            shell=True,
