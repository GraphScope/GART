...
class SegmentHeader : public BlockHeader {
 public:
  ...
  uintptr_t get_head() const { return head; }
  ...
  void fill(uintptr_t head, order_t order, segid_t segid) {
...
class N2OBlockHeader : public BlockHeader {
 public:
  ...
  void set_prev_pointer(uintptr_t prev_pointer) {
    this->prev_pointer = prev_pointer;
  }

  void fill(order_t order, Type type, vertex_t vid, timestamp_t creation_time,
...
class EdgeLabelEntry {
 public:
  ...
  uintptr_t get_pointer(dir_t dir = EOUT) const { return pointers[dir]; }
...
class EdgeLabelBlockHeader : public N2OBlockHeader {
 public:
  ...
  void fill(order_t order, uint64_t segid, timestamp_t creation_time,
            uintptr_t prev_pointer) {
    N2OBlockHeader::fill(order, Type::EDGE_LABEL, segid, creation_time,
                         prev_pointer);
    clear();
  }
...
class EdgeBlockHeader : public N2OBlockHeader {
 public:
  ...
  bool has_space(EdgeEntry entry, size_t num_entries,
    ...
    if (sizeof(*this) + (num_entries + 1) * sizeof(entry) + data_length +
            entry.get_length() + bloom_filter_size >
        get_block_size())
      return false;
    else
      return true;
  ...
  void set_num_entries_data_length_atomic(size_t num_entries,
                                          size_t data_length) {
    Int128Union new_val;
    ...
    while (!__sync_bool_compare_and_swap(&tail.int128, tail.int128,
                                         new_val.int128))
      _mm_pause();
  }
  ...
  std::pair<size_t, size_t> get_num_entries_data_length_atomic() {
    ...
    // should be inlined as "lock cmpxchg16b"
    // cur_val.int128 = __sync_val_compare_and_swap(&tail.int128,
    // new_val.int128, tail.int128);
    Int128Union cur_val;
  ...
  void fill(order_t order, vertex_t vid, timestamp_t creation_time,
            uintptr_t prev_pointer, timestamp_t committed_time) {
...
class VegitoEdgeBlockHeader : public BlockHeader {
 public:
  ...
  void set_prev_num_entries(size_t prev_num_entries) {
    this->prev_num_entries = prev_num_entries;
  }
  ...
  bool has_space() const {
    if (sizeof(*this) + (num_entries + 1) * sizeof(VegitoEdgeEntry) >
        get_vegito_block_size())
      return false;
    else
      return true;
  }
  ...
  void fill(order_t order, uintptr_t prev_pointer, size_t prev_num_entries) {
...
class EpochBlockHeader : public BlockHeader {
 public:
  ...
  void set_latest_epoch(timestamp_t latest_epoch) {
    this->latest_epoch = latest_epoch;
  }
  ...
  bool has_space() const {
    if ((sizeof(*this) + (num_entries + 1) * sizeof(VegitoEpochEntry)) >
        get_block_size())
      return false;
    else
      return true;
  ...
  order_t get_order() const {
    auto result = BlockHeader::get_order();
    return result;
  }
...
class VegitoSegmentHeader : public BlockHeader {
 public:
  ...
  uintptr_t alloc(order_t edge_num_order, size_t edge_prop_size) {
    ...
    size_t require_edge_num =
        sizeof(VegitoEdgeBlockHeader) / sizeof(VegitoEdgeEntry) + edge_num;
    size_t ret = __sync_fetch_and_add(&allocated_edge_num, require_edge_num);

    if (sizeof(*this) + (ret + require_edge_num) *
                            (sizeof(VegitoEdgeEntry) + edge_prop_size) >
        get_block_size())
      return 0;
    else
      return head + sizeof(*this) + ret * sizeof(VegitoEdgeEntry);
  }
  ...
  void set_epoch_table(uint32_t idx, uintptr_t epoch_table) {
    epoch_tables[idx] = epoch_table;
  }
