...
class SegTransaction {
  ...
  enum class ThreadStatus { WORKING, STEALING };
 ...
 public:
  ...
  SegTransaction(SegTransaction&& txn)
      : graph(txn.graph),
        ...
        edge_block_num_entries_data_length_cache(
            std::move(txn.edge_block_num_entries_data_length_cache)),
 ...
 private:
  ...
  void check_writable() {
    if (!batch_update && !trace_cache)
      throw std::invalid_argument(
          "The transaction is read-only without cache.");
  }
  ...
  bool has_write_lock(segid_t seg_id) {
    return (acquired_seg_write_locks.find(seg_id) !=
            acquired_seg_write_locks.end());
  }
  ...
  void recover_lock_status(segid_t seg_id, vertex_t vid, bool recover_read,
                           bool recover_write, bool recover_vertex) {
    if (recover_read) {
  ...
  void ensure_no_confict(vertex_t vertex_id) {
    auto header = graph.block_manager.convert<VertexBlockHeader>(
        graph.vertex_ptrs[vertex_id]);
    if (header && cmp_timestamp(header->get_creation_time_pointer(),
                                read_epoch_id, local_txn_id) > 0)
      throw RollbackExcept(
          "Write-write confict on: " + std::to_string(vertex_id) + ".");
  }
  ...
  void set_num_entries_data_length_cache(EdgeBlockHeader* edge_block,
    ...
    if (batch_update)
      ...
      edge_block_num_entries_data_length_cache[edge_block] = {num_entries,
                                                              data_length};
  }
