/*
 *
 * The file seggraph/src/segment_transaction.cpp is referred and derived from
 * project livegraph,
 *
...
using EdgeEntry = seggraph::EdgeEntry;
...
vertex_t SegTransaction::new_vertex(bool use_recycled_vertex) {
  ...
  } else if (!use_recycled_vertex ||
             (!graph.recycled_vertex_ids.try_pop(vertex_id))) {
...
void SegTransaction::put_vertex(vertex_t vertex_id, std::string_view data) {
  ...
  vertex_block->fill(order, vertex_id, write_epoch_id, prev_pointer,
                     data.data(), data.size());
  ...
  if (batch_update) {
    ...
    timestamps_to_update.emplace_back(vertex_block->get_creation_time_pointer(),
...
bool SegTransaction::del_vertex(vertex_t vertex_id, bool recycle) {
  ...
  bool ret = false;
  auto prev_vertex_block =
      graph.block_manager.convert<VertexBlockHeader>(prev_pointer);
  if (prev_vertex_block &&
      prev_vertex_block->get_length() != prev_vertex_block->TOMBSTONE) {
    ret = true;
    ...
    auto vertex_block = graph.block_manager.convert<VertexBlockHeader>(pointer);
    vertex_block->fill(order, vertex_id, write_epoch_id, prev_pointer, nullptr,
                       vertex_block->TOMBSTONE);

    if (!batch_update) {
      block_cache.emplace_back(pointer, order);
      timestamps_to_update.emplace_back(
          vertex_block->get_creation_time_pointer(),
          SegGraph::ROLLBACK_TOMBSTONE);
      vertex_ptr_cache[vertex_id] = pointer;
    } else {
      graph.vertex_ptrs[vertex_id] = pointer;
...
std::string_view SegTransaction::get_vertex(vertex_t vertex_id) {
  ...
  auto vertex_block = graph.block_manager.convert<VertexBlockHeader>(pointer);
  while (vertex_block) {
    if (cmp_timestamp(vertex_block->get_creation_time_pointer(), read_epoch_id,
                      local_txn_id) <= 0)
      break;
...
std::pair<EdgeEntry*, char*> SegTransaction::locate_edge_in_block(
  ...
  for (size_t i = 0; i < num_entries; i++) {
    ...
    if (entries->get_dst() == dst &&
        cmp_timestamp(entries->get_creation_time_pointer(), read_epoch_id,
                      local_txn_id) <= 0 &&
        cmp_timestamp(entries->get_deletion_time_pointer(), read_epoch_id,
                      local_txn_id) > 0) {
      return {entries, data};
    }
    entries++;
...
void SegTransaction::update_edge_label_block(segid_t segid, label_t label,
                                             dir_t dir,
  ...
  if (!edge_label_block || !edge_label_block->append(label_entry)) {
    auto num_entries =
        edge_label_block ? edge_label_block->get_num_entries() : 0;
    auto size = sizeof(EdgeLabelBlockHeader) +
                (1 + num_entries) * sizeof(EdgeLabelEntry);
    auto order = size_to_order(size);

    auto new_pointer = graph.block_manager.alloc(order);

    auto new_edge_label_block =
        graph.block_manager.convert<EdgeLabelBlockHeader>(new_pointer);
    new_edge_label_block->fill(order, segid, write_epoch_id, pointer);

    if (!batch_update) {
      block_cache.emplace_back(new_pointer, order);
      timestamps_to_update.emplace_back(
          new_edge_label_block->get_creation_time_pointer(),
          SegGraph::ROLLBACK_TOMBSTONE);
    }
...
void SegTransaction::put_edge(vertex_t src, label_t label, dir_t dir,
                              vertex_t dst, std::string_view edge_data,
                              bool force_insert) {
  check_valid();
  check_writable();
  check_vertex_id(src);
...
start:
  if (batch_update) {
    ...
    if (cache_iter != segment_ptr_cache.end()) {
      ...
      seg_pointer = locate_segment(segid, label, dir);
      test_seg_pointer = seg_pointer;
      segment_ptr_cache.emplace_hint(cache_iter, std::make_pair(segid, label),
                                     seg_pointer);
    }
    pointer = locate_block_in_segment(seg_pointer, segidx);
  }
  ...
  auto [num_entries, data_length] =
      edge_block ? get_num_entries_data_length_cache(edge_block)
  ...
  if (!segment) {
    ...
    if (seg_pointer == test_seg_pointer) {
      ...
      if (batch_update) {
        ...
        recover_lock_status(segid, src, recover_read, recover_write,
                            recover_vertex);
      }
      goto start;
    }
  ...
  if (!edge_block || !edge_block->has_space(entry, num_entries, data_length)) {
    ...
    if (!new_edge_block_pointer) {
      ...
      if (test_seg_pointer == cur_seg_pointer) {
        ...
        merge_segment(segment, new_segment, segidx, entry, &pointer,
                      &edge_block);

        if (!batch_update) {
          segment_to_be_freed_cache.emplace_back(seg_pointer,
                                                 segment->get_order());
      ...
      new_edge_block->fill(order, src, write_epoch_id,
                           graph.block_manager.NULLPOINTER, write_epoch_id);
      ...
      if (edge_block) {
        ...
        for (size_t i = 0; i < num_entries; i++) {
          entries--;
          auto edge = new_edge_block->append(
              *entries, data, bloom_filter);  // direct update size
          if (!batch_update && edge->get_creation_time() == -local_txn_id)
            timestamps_to_update.emplace_back(edge->get_creation_time_pointer(),
                                              SegGraph::ROLLBACK_TOMBSTONE);
          data += entries->get_length();
        }
      }
      ...
      if (!batch_update) {
        timestamps_to_update.emplace_back(
            new_edge_block->get_creation_time_pointer(),
            SegGraph::ROLLBACK_TOMBSTONE);
        region_ptr_cache[segment->get_region_ptr_pointer(segidx)] =
            new_edge_block_pointer;
      } else {
  ...
  if (!force_insert) {
    ...
    while (prev_edge_block) {
      auto [num_entries, data_length] =
          get_num_entries_data_length_cache(prev_edge_block);
      if (num_entries > 0) {
        auto prev_edge = locate_edge_in_block(dst, prev_edge_block, num_entries,
                                              data_length);
        if (prev_edge.first) {
          prev_edge.first->set_deletion_time(write_epoch_id);
          if (!batch_update)
            timestamps_to_update.emplace_back(
                prev_edge.first->get_deletion_time_pointer(),
                SegGraph::ROLLBACK_TOMBSTONE);
          break;
        }
      }
      prev_edge_block = graph.block_manager.convert<EdgeBlockHeader>(
          prev_edge_block->get_prev_pointer());
    }
  }

  // insert edge
  std::tie(num_entries, data_length) =
      get_num_entries_data_length_cache(edge_block);
  auto edge = edge_block->append_without_update_size(entry, edge_data.data(),
                                                     num_entries, data_length);
  set_num_entries_data_length_cache(edge_block, num_entries + 1,
                                    data_length + entry.get_length());
  if (!batch_update)
    timestamps_to_update.emplace_back(edge->get_creation_time_pointer(),
                                      SegGraph::ROLLBACK_TOMBSTONE);
...
bool SegTransaction::del_edge(vertex_t src, label_t label, dir_t dir,
                              vertex_t dst) {
  check_valid();
  check_writable();
  check_vertex_id(src);
  ...
  while (edge_block) {
    ...
    if (edge.first) {
      ...
      if (!batch_update)
        timestamps_to_update.emplace_back(
            edge.first->get_deletion_time_pointer(),
            SegGraph::ROLLBACK_TOMBSTONE);
      break;
    }
    edge_block = graph.block_manager.convert<EdgeBlockHeader>(
...
std::string_view SegTransaction::get_edge(vertex_t src, label_t label,
                                          dir_t dir, vertex_t dst) {
  check_valid();
...
bool SegTransaction::merge_segment(SegmentHeader* old_seg,
  ...
  for (int i = 0; i < VERTEX_PER_SEG; i++) {
    ...
    while (region_ptr) {
      ...
      auto merged_block =
          graph.block_manager.convert<EdgeBlockHeader>(region_ptr);
      ...
      for (size_t j = 0; j < num_merged_entries; j++) {
        merged_entries--;
        // if (cmp_timestamp(merged_entries->get_deletion_time_pointer(),
        // read_epoch_id) > 0)
        {
          new_num_entries++;
    ...
    auto merged_size = (pointer != nullptr && i == segidx)
                           ? sizeof(EdgeBlockHeader) +
                                 (new_num_entries + 1) * sizeof(EdgeEntry) +
                                 new_data_length + entry.get_length()
                           : sizeof(EdgeBlockHeader) +
                                 new_num_entries * sizeof(EdgeEntry) +
                                 new_data_length;
    auto merged_order = size_to_order(merged_size);

    if (merged_order > EdgeBlockHeader::BLOOM_FILTER_PORTION &&
        merged_size + (1ul << (merged_order -
                               EdgeBlockHeader::BLOOM_FILTER_PORTION)) >=
            (1ul << EdgeBlockHeader::BLOOM_FILTER_THRESHOLD)) {
      merged_size +=
          1ul << (merged_order - EdgeBlockHeader::BLOOM_FILTER_PORTION);
    }
    ...
    auto new_edge_block =
        graph.block_manager.convert<EdgeBlockHeader>(new_edge_block_pointer);
    new_edge_block->fill(
        merged_order, VERTEX_PER_SEG * old_seg->get_segment_id() + i,
        new_creation_time, graph.block_manager.NULLPOINTER, write_epoch_id);
    ...
    for (int j = merged_edge_blocks.size() - 1; j >= 0; j--) {
      ...
      auto merged_block =
          graph.block_manager.convert<EdgeBlockHeader>(merged_block_ptr);
      size_t num_merged_entries = merged_block->get_num_entries();
      auto merged_entries = merged_block->get_entries();
      ...
      for (size_t k = 0; k < num_merged_entries; k++) {
        ...
        {
          auto merged_edge =
              new_edge_block->append(*merged_entries, merged_data,
                                     new_bloom_filter);  // direct update size
          if (!batch_update &&
              merged_edge->get_creation_time() == -local_txn_id)
            timestamps_to_update.emplace_back(
                merged_edge->get_creation_time_pointer(),
                SegGraph::ROLLBACK_TOMBSTONE);
        }
  ...
  for (segid_t segid = 0; segid < graph.get_max_seg_id(); segid++) {
    ...
    if (segment) {
      ...
      if (!merge_segment(segment, new_segment, -1, entry, nullptr, nullptr)) {
        ...
        new_segment =
            graph.block_manager.convert<SegmentHeader>(new_seg_pointer);
        new_segment->fill(new_seg_pointer, segment->get_order() + 1, segid);
